...
void List_TYPE_insert_at(Arena *arena, List_TYPE *list, TYPE item, i32 index) {
    ...
    if (index > list->length - 1 || index < 0) {
        ...
        List_TYPE_append(arena, list, item);
    } else if (index == 0) {
        List_TYPE_prepend(arena, list, item);
    }
    list->length += 1;
    ...
    while (iter) {
        if (count == index) {
            ...
            Node_TYPE *new_node = Node_TYPE_create(arena, item);
            if (next_node) {
                iter->next = new_node;
...
void List_TYPE_append(Arena *arena, List_TYPE *list, TYPE item) {
    ...
    iter->next = Node_TYPE_create(arena, item);
    list->length += 1;
    return;
}
...
void List_TYPE_destroy(Arena *arena, List_TYPE *list) {
    ...
    while (iter) {
        ...
        iter = iter->next;
        Node_TYPE_destroy(arena, &destroyer);
    }
    // free(*list);
    list->head = NULL;
    list->length = 0;
