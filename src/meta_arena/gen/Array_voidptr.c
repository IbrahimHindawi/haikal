...
#include "Array_voidptr.h"

Array_voidptr Array_voidptr_reserve(Arena *arena, u64 length) {
    Array_voidptr array = {0};
    if (length == 0) {
        return array;
    }
    array.length = length;
    array.data = arenaPushArray(arena, voidptr, length);
    // array.data = malloc(sizeof(voidptr) * length);
    if (array.data == NULL) {
        printf("haikal::Memory Allocation Failure!\n");
        exit(-1);
    }
    // #ifdef DEBUG
    memset(array.data, 0, sizeof(voidptr) * length);
    // #endif
    array.border = length;
    return array;
}
...
voidptr *Array_voidptr_resize(Arena *arena, Array_voidptr *array) {
    ...
    array->border *= 2;
    array->data = arenaRealloc(arena, voidptr, array->border, array->data, old_border);
    // array->data = realloc(arena, array->data, sizeof(voidptr) * array->border);
    if (array->data == NULL) {
    ...
    if (array->border > old_border) {
        ...
        memset(cursor + (sizeof(voidptr) * old_border), 0, sizeof(voidptr) * border_difference);
    }
    // #endif
    return array->data;
}
...
voidptr *Array_voidptr_append(Arena *arena, Array_voidptr *array, voidptr elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        array->data = arenaPushArray(arena, voidptr, array->border);
        // array->data = realloc(NULL, sizeof(voidptr) * array->border);
    ...
    if (array->length == array->border ) {
        ...
        Array_voidptr_resize(arena, array);
    }
    array->length += 1;
    array->data[array->length - 1] = elem;
    // char *cursor = (char *)array->data;
