...
Array_i32 Array_i32_reserve(Arena *arena, u64 length) {
    ...
    array.length = length;
    array.data = arenaPushArray(arena, i32, length);
    // array.data = malloc(sizeof(i32) * length);
    if (array.data == NULL) {
        printf("haikal::Memory Allocation Failure!\n");
        exit(-1);
    }
    // #ifdef DEBUG
    memset(array.data, 0, sizeof(i32) * length);
    // #endif
    array.border = length;
    return array;
}
...
i32 *Array_i32_resize(Arena *arena, Array_i32 *array) {
    ...
    array->border *= 2;
    array->data = arenaRealloc(arena, i32, array->border, array->data, old_border);
    // array->data = realloc(arena, array->data, sizeof(i32) * array->border);
    if (array->data == NULL) {
    ...
    if (array->border > old_border) {
        ...
        memset(cursor + (sizeof(i32) * old_border), 0, sizeof(i32) * border_difference);
    }
    // #endif
    return array->data;
}
...
i32 *Array_i32_append(Arena *arena, Array_i32 *array, i32 elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        array->data = arenaPushArray(arena, i32, array->border);
        // array->data = realloc(NULL, sizeof(i32) * array->border);
