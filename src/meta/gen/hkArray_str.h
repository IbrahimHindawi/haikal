...
hkArray_str hkarray_str_create(size_t length) {
    hkArray_str array = {0};
    if (length == 0) {
        array.unit_size = sizeof(str);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_str_destroy(hkArray_str *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
str *hkarray_str_resize(hkArray_str *array, size_t new_border) {
...
str *hkarray_str_append(hkArray_str *array, str elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_str_resize(array, array->border);
...
int hkarray_str_is_empty(hkArray_str *array) {
