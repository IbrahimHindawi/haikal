...
hkArray_i16 hkarray_i16_create(size_t length) {
    hkArray_i16 array = {0};
    array.unit_size = sizeof(i16);
    array.length = length;
...
void hkarray_i16_destroy(hkArray_i16 *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
void *hkarray_i16_resize(hkArray_i16 *array, size_t new_border) {
...
void *hkarray_i16_append(hkArray_i16 *array, void *elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_i16_resize(array, array->border);
        char *cursor = (char *)array->data;
...
int hkarray_i16_is_empty(hkArray_i16 *array) {
