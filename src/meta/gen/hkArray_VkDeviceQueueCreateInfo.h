#pragma once
/* hkArray_VkDeviceQueueCreateInfo */
#include <core.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
structdef(hkArray_VkDeviceQueueCreateInfo) {
    VkDeviceQueueCreateInfo *data;
    size_t unit_size;
    size_t length;
    size_t border;
};

hkArray_VkDeviceQueueCreateInfo hkarray_VkDeviceQueueCreateInfo_create(size_t length) {
    hkArray_VkDeviceQueueCreateInfo array = {0};
    if (length == 0) {
        array.unit_size = sizeof(VkDeviceQueueCreateInfo);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_VkDeviceQueueCreateInfo_destroy(hkArray_VkDeviceQueueCreateInfo *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
VkDeviceQueueCreateInfo *hkarray_VkDeviceQueueCreateInfo_resize(hkArray_VkDeviceQueueCreateInfo *array, size_t new_border) {
...
VkDeviceQueueCreateInfo *hkarray_VkDeviceQueueCreateInfo_append(hkArray_VkDeviceQueueCreateInfo *array, VkDeviceQueueCreateInfo elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_VkDeviceQueueCreateInfo_resize(array, array->border);
...
int hkarray_VkDeviceQueueCreateInfo_is_empty(hkArray_VkDeviceQueueCreateInfo *array) {
