...
hkArray_strptr hkarray_strptr_create(size_t length) {
    hkArray_strptr array = {0};
    if (length == 0) {
        array.unit_size = sizeof(strptr);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_strptr_destroy(hkArray_strptr *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
strptr *hkarray_strptr_resize(hkArray_strptr *array, size_t new_border) {
...
strptr *hkarray_strptr_append(hkArray_strptr *array, strptr elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_strptr_resize(array, array->border);
...
int hkarray_strptr_is_empty(hkArray_strptr *array) {
