...
hkArray_u32 hkarray_u32_create(size_t length) {
    hkArray_u32 array = {0};
    array.unit_size = sizeof(u32);
    array.length = length;
...
void hkarray_u32_destroy(hkArray_u32 *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
void *hkarray_u32_resize(hkArray_u32 *array, size_t new_border) {
...
void *hkarray_u32_append(hkArray_u32 *array, void *elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_u32_resize(array, array->border);
        char *cursor = (char *)array->data;
...
int hkarray_u32_is_empty(hkArray_u32 *array) {
