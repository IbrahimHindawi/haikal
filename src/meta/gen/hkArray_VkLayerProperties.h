...
structdef(hkArray_VkLayerProperties) {
...
hkArray_VkLayerProperties hkarray_VkLayerProperties_create(size_t length) {
    hkArray_VkLayerProperties array = {0};
    if (length == 0) {
        array.unit_size = sizeof(VkLayerProperties);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_VkLayerProperties_destroy(hkArray_VkLayerProperties *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
VkLayerProperties *hkarray_VkLayerProperties_resize(hkArray_VkLayerProperties *array, size_t new_border) {
...
VkLayerProperties *hkarray_VkLayerProperties_append(hkArray_VkLayerProperties *array, VkLayerProperties elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_VkLayerProperties_resize(array, array->border);
...
int hkarray_VkLayerProperties_is_empty(hkArray_VkLayerProperties *array) {
