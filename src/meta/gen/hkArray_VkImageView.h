...
hkArray_VkImageView hkarray_VkImageView_create(size_t length) {
    hkArray_VkImageView array = {0};
    if (length == 0) {
        array.unit_size = sizeof(VkImageView);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_VkImageView_destroy(hkArray_VkImageView *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
VkImageView *hkarray_VkImageView_resize(hkArray_VkImageView *array, size_t new_border) {
...
VkImageView *hkarray_VkImageView_append(hkArray_VkImageView *array, VkImageView elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_VkImageView_resize(array, array->border);
...
int hkarray_VkImageView_is_empty(hkArray_VkImageView *array) {
