...
structdef(hkArray_VkPhysicalDevice) {
...
hkArray_VkPhysicalDevice hkarray_VkPhysicalDevice_create(size_t length) {
    hkArray_VkPhysicalDevice array = {0};
    if (length == 0) {
        array.unit_size = sizeof(VkPhysicalDevice);
        array.length = length;
        array.data = NULL;
        array.border = 0;
        return array;
...
void hkarray_VkPhysicalDevice_destroy(hkArray_VkPhysicalDevice *array) {
    array->border = 0;
    array->length = 0;
    array->unit_size = 0;
    free(array->data);
...
VkPhysicalDevice *hkarray_VkPhysicalDevice_resize(hkArray_VkPhysicalDevice *array, size_t new_border) {
...
VkPhysicalDevice *hkarray_VkPhysicalDevice_append(hkArray_VkPhysicalDevice *array, VkPhysicalDevice elem) {
    if (array->length == 0 && array->border == 0) { 
        array->length += 1;
        array->border += 1;
        hkarray_VkPhysicalDevice_resize(array, array->border);
...
int hkarray_VkPhysicalDevice_is_empty(hkArray_VkPhysicalDevice *array) {
